...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        for b in dets:
            ...
            Rect_Point = np.array(
                [[b[5], b[6]], [b[7], b[8]], [b[13] - (b[13]-b[11])/2, b[14]-(b[14]-b[12])/2]],
                dtype=np.float32)
            rect_matrix = cv2.getAffineTransform(Rect_Point, facedstPoint)
            fixed_face = cv2.warpAffine(frame, rect_matrix, (frame.shape[1], frame.shape[0]))  # width. height
            ...
            left_eye_Point = np.array(
                [[l_start_x, l_start_y], [l_end_x, l_start_y], [l_start_x, l_end_y], [l_end_x, l_end_y]],
                dtype=np.float32)
        ...
        """# save image
        if args.save_image:
            save_dir = "D:/Dataset/NTHU-DDD-modified/Training_Evaluation_Dataset/Training Dataset"
            name = save_dir + subject + condition + scenario + i + ".jpg"
